...
BEGIN
	...
	INSERT INTO @Appointment([Id],
		...
		INNER JOIN	[Superoffice7].[CRM7].[UDAPPNTSMALL] [UDAPPNTSMALL]
		...
		LEFT JOIN	[Superoffice7].[CRM7].[UDAPPNTLARGE] [UDAPPNTLARGE]
		...
		WHERE		[Appointment].[Status] <> 13
		--AND			[Appointment].do_by BETWEEN DATEADD(MONTH, -6, GETDATE()) AND DATEADD(MONTH, +6, GETDATE())
