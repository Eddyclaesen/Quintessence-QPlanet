CREATE PROCEDURE [dbo].[ProjectCandidate_AssistantOverview]
...
BEGIN
	...
	SELECT		[ProjectCandidateView].[Id]															AS  [Id],							
				...
				CASE WHEN ([ProjectCandidateView].[Extra1] is not null OR
				[ProjectCandidateView].[Extra2] is not null)
				...
				[ProjectCandidateView].[LeafletSentDate]											AS  [LeafletSentDate],				
				...
				[ProjectTypeCategoryView].[Code]													AS	[Type],
				...
				[CrmReplicationAppointment].[AppointmentDate]										AS  [Date],
				...
				[dbo].[ProjectCandidate_GetLastTheoremListIsCompleted]([ProjectCandidateView].[Id])	AS	[CulturalFit],
	...
	WHERE		((CrmReplicationAppointment.AppointmentDate IS NULL OR @StartDate IS NULL OR CrmReplicationAppointment.AppointmentDate >= @StartDate)
				AND	(CrmReplicationAppointment.AppointmentDate IS NULL OR @EndDate IS NULL OR CrmReplicationAppointment.AppointmentDate <= @EndDate))
				AND	(@CustomerAssistantId IS NULL OR [CustomerAssistant].[Id] = @CustomerAssistantId)	
				AND [ProjectCandidateView].[FollowUpDone] = 0
				OR ProjectCandidateView.Id = @ProjectCandidateId
END