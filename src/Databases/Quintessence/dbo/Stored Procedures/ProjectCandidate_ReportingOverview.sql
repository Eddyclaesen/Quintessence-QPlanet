...
BEGIN
	...
	SELECT		[ProjectCandidateView].[Id]														AS  [Id],							
				...
				CASE WHEN 
					DATEADD(day, AssessmentDevelopmentProjectView.ReportDeadlineStep, [Appointment].AppointmentDate) > [ProjectCandidateView].[ReportDeadline]
					THEN DATEADD(day, AssessmentDevelopmentProjectView.ReportDeadlineStep, [Appointment].AppointmentDate)
					ELSE [ProjectCandidateView].[ReportDeadline]
				END																					AS  [ReportDeadline],
	...
	WHERE		(@ProjectCandidateId IS NULL OR [ProjectCandidateView].[Id] = @ProjectCandidateId)
		AND		(@StartDate IS NULL OR (CONVERT(DATE,DATEADD(day, AssessmentDevelopmentProjectView.ReportDeadlineStep, [Appointment].AppointmentDate)) <= CONVERT(DATE,@StartDate) 
				AND @StartDate >= ProjectCandidateView.ReportDeadline
				AND [ReportStatus].[Id] NOT IN (4,5)))
