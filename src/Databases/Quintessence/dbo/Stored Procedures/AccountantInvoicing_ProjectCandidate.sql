...
BEGIN
	...
	SELECT		[ProjectCandidateView].[Id]															AS  [Id],							
				...
				ISNULL(ISNULL([CrmReplicationAppointment].[AppointmentDate], [ProjectCandidateView].CancelledDate),[ProjectCandidateView].Audit_CreatedOn) AS  [Date],
				...
				[ProjectCandidateView].[FinancialEntityId]											AS	[FinancialEntityId]
	...
	WHERE		(
					...
					(
						(
							[ProjectCandidateView].[InvoicedDate] >= DATEADD(MONTH, DATEDIFF(MONTH, 0, @Date)-1, 0)
						)
						OR
						(
