...
{
    public class TeamLeaderMeetingDuplication : ReplicationBase, IJobDefinition
    {
        public void Run(IJobControllerService jobControllerService)
        {
            ...
            catch (Exception ex)
            {
                if(jobControllerService != null)
                    jobControllerService.WriteError("TeamLeaderMeetingDuplicator", ex);
                throw;
            }
        }
        ...
        internal void Run(IJobControllerService jobControllerService, IDuplicationDataAccess duplicationDataAccess, ITeamLeaderAccess teamLeaderAccess)
        {
            ...
            {
                if (jobControllerService != null)
                    jobControllerService.WriteInformation("TeamLeaderMeetingDuplicator started");

                new TeamLeaderMeetingDuplicator(jobControllerService, duplicationDataAccess, teamLeaderAccess).RunAsync().Wait();
            }
            catch (Exception ex)
            {
                if (jobControllerService != null)
            ...
            {
                if (jobControllerService != null)
                    jobControllerService.WriteInformation("TeamLeaderMeetingDuplicator ended");
            }
        }
    }

    public class TeamLeaderMeetingDuplicator : Duplicator
    {
        public TeamLeaderMeetingDuplicator(IJobControllerService jobControllerService, IDuplicationDataAccess duplicationDataAccess, ITeamLeaderAccess teamLeaderAccess)
        ...
        public async System.Threading.Tasks.Task RunAsync()
        {
            ...
            try
            {
                ...
                int pageNumber = 0;
                int affected = 0;
                bool meetingsFetched;
                DateTime? fromDate = new DateTime(2012, 12, 02);
                DateTime? toDate = DateTime.Today.AddYears(1);

                do
                {
                    IEnumerable<TeamLeaderMeeting> teamLeaderMeetings = await _teamLeaderAccess.RetrieveMeetings(batchSize, pageNumber, fromDate, toDate);

                    List<TeamLeaderMeeting> meetings = new List<TeamLeaderMeeting>(teamLeaderMeetings);
                    meetingsFetched = meetings.Any();

                    // Sync with db
                    affected += _duplicationDataAccess.RegisterMeetings(meetings);
                    pageNumber++;

                } while (meetingsFetched);

                RegisterJobEnd(jobHistoryId, true, String.Format("{0} Meeting(s) duplicated", affected));
            }
            catch (Exception ex)
            {
